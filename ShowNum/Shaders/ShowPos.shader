    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata_base v)
            {
                ...
                o.object_pos = mul(unity_ObjectToWorld, float4(0, 0, 0, 1)).xyz;
            ...
            float4 frag (v2f i) : SV_Target
            {
                ...
                int posIndex = 2 - int(pos_uv.y * 3);
                float posValue = i.object_pos[posIndex];
                if (_Value) posValue = _Value;
                num_col = frag_number(
                    float2(frac(pos_uv.x), frac(pos_uv.y * 3)),
                    float2(i.pos_uv.x, i.pos_uv.y * 3),
                    posValue, _Digits, _NSmall, _Unit);
